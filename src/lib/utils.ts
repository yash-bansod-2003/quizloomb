type QuestionType = "mcq" | "true_false" | "written" | "multi_select";
...
export function parseQuizFile(content: string): ParseResult {
  ...
  const flushQuestion = (lines: string[], lineStart: number) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith("type:")) {
        ...
        if (!["mcq", "true_false", "written", "multi_select"].includes(type)) {
        ...
        errors.push(`Line ${lineNumber}: Unknown or misplaced line "${line}"`);
      }
    }
