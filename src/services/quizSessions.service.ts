...
class QuizSessionsService {
  constructor(
    private readonly quizSessionsRepository: Repository<QuizSession>,
  ) {}

  async create(
    createQuizSessionDto: DeepPartial<QuizSession>,
    options?: SaveOptions,
  ) {
    const quizSession =
      this.quizSessionsRepository.create(createQuizSessionDto);
    return await this.quizSessionsRepository.save(quizSession, options);
  }

  async findAll(
    options?: FindManyOptions<QuizSession>,
  ): Promise<QuizSession[]> {
    return await this.quizSessionsRepository.find(options);
  }

  async findOne(
    options: FindOneOptions<QuizSession>,
  ...
  async update(
    ...
    return await this.quizSessionsRepository.update(
      criteria,
